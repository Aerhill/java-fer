...
public class ArrayIndexedCollection extends Collection {
     ...
     * are returned by the collection's iterator. If user provides initial capacity
     * smaller than collection size, it will be ignored and capacity will 
     * be equal to collection size.
     *
     * @param other
     *            the collection whose elements are to be placed into this list
     * @param initialCapacity
	...
	if (index < 0 || index >= size){
	    throw new IndexOutOfBoundsException("Specified index is not in this collection");
	}
	...
	if (value == null) {
	    throw new IllegalArgumentException("Storing null values is not supported");
	}
	if (position < 0 || position >= size) {
	    throw new IndexOutOfBoundsException("You specified invalid position, please add something within bounds from 0 to " + (size-1));
	}
	if (size + 1 >= capacity) {
	    elements = resize(elements);
	}

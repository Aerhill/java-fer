...
public class CStringTests {
	...
	public void testFromStringWithRegularValue() {
		CString string1 = new CString(String.valueOf("Štefica Štefica Štefica")
				.toCharArray(), 7, 9);
		CString string2 = new CString(String.valueOf(" Štefica i Marko  ")
				.toCharArray());
		...
		assertEquals(
				"Expected '0'",
				0,
				string1.toString().compareTo(
						CString.fromString(" Štefica ").toString()));
		assertEquals(
				"Expected '0'",
				0,
				string2.toString().compareTo(
						CString.fromString(" Štefica i Marko  ").toString()));
		assertEquals(
				"Expected '0'",
				0,
				string3.toString().compareTo(
						CString.fromString("tefica ").toString()));
	}

	@Test
	public void testContainsWithRegularValue() {
		CString string = new CString(String.valueOf(" Štefica i Marko  ")
				.toCharArray());
	...
	public void testContainsWithNullValue() {
		// must throw!
		new CString(String.valueOf(" Štefica i Marko  ").toCharArray())
				.contains(null);
	}
	...
	public void testSubstringWithValidArguments() {
		...
		assertEquals("Expected that they are the same.", 0,
				" Štefica".compareTo(string.substring(0, 8).toString()));
		assertEquals("Expected that they are the same.", 0,
				"Marko ".compareTo(string.substring(11, 17).toString()));
		assertEquals("Expected that they are the same.", 0,
				"ica".compareTo(string.substring(5, 8).toString()));
		assertEquals("Expected that they are the same.", 0,
				" ".compareTo(string.substring(0, 1).toString()));
	}
	...
	public void testAllWithValidArgumetns() {
		CString string1 = CString.fromString("Štefica").add(
				CString.fromString(" i "));
		CString string2 = CString.fromString("Marko");

		assertEquals("Expected 'Štefica i '", "Štefica i ", string1.toString());
		assertEquals("Expected 'Štefica i Marko'.", "Štefica i Marko", string1
				.add(string2).toString());
	}

	@Test
	public void testReplaceAllWithCharsAsArguments() {
		CString string1 = CString
				.fromString("# Štefica# i Marko # se vole. # #");
		CString string2 = string1.replaceAll('#', '*');
		CString string3 = string2.replaceAll('$', '*');
		CString string4 = string3.replaceAll(' ', '$');

		assertEquals("I should be full of stars.",
				"* Štefica* i Marko * se vole. * *", string2.toString());
		assertEquals("I should be also full of stars.",
				"* Štefica* i Marko * se vole. * *", string3.toString());
		assertEquals("I should be also full of stars and dollaz.",
				"*$Štefica*$i$Marko$*$se$vole.$*$*", string4.toString());
	}

	@Test(expected = NullPointerException.class)
	public void testReplaceAllWithNullAsValue1() {
		// must throw!
		CString.fromString("Štefica i Marko šetaju se šumom.").replaceAll(null,
	...
	public void testReplaceAllWithNullAsValue2() {
		// must throw!
		CString.fromString("Štefica i Marko šetaju se šumom.").replaceAll(
				CString.fromString("Marko"), null);
	}
	...
	public void testReplaceAllWithCStringsAsArguments() {
		...
		CString result1 = string1.replaceAll(CString.fromString("Štefica"),
				CString.fromString("Marica"));
		CString result2 = string1.replaceAll(CString.fromString("Marko"),

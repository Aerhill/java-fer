...
public class TrigonometricServlet extends HttpServlet {
	private static final long serialVersionUID = 7093781056802615141L;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Map<Integer, Double[]> trig = new HashMap<>();
		for (int i = a; i <= b; i++) {
			trig.put(i, new Double[] { Math.sin(Math.toRadians(i)), Math.cos(Math.toRadians(i)) });
		}
		req.getSession(true).setAttribute("trig", trig);

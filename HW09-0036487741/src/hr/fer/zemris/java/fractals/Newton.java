...
public class Newton {
	...
	public static class PosaoIzracuna extends RecursiveAction {
		...
		public void compute() {
			...
			invokeAll(
					new PosaoIzracuna(reMin, reMax, imMin, imMax, width, height,
							yMin, yMin + (yMax - yMin) / 2, m, data),
					new PosaoIzracuna(reMin, reMax, imMin, imMax, width, height,
							yMin + (yMax - yMin) / 2 + 1, yMax, m, data));
		}

		public void computeDirect() {

			Mandelbrot.calculate(reMin, reMax, imMin, imMax, width, height, m,
					yMin, yMax, data);
	...
	public static class MojProducer implements IFractalProducer {
		...
		public void produce(double reMin, double reMax, double imMin,
			...
			pool.invoke(new PosaoIzracuna(reMin, reMax, imMin, imMax, width,
					height, 0, height - 1, m, data));
			pool.shutdown();
